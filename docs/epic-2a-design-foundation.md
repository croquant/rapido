...
| Client-side interactivity | **Alpine.js 3.x** (self-hosted at `static/js/alpine.min.js`) as a complement to HTMX for purely-local UI state (dialog open/close, dropdown toggles, simple show/hide). No `onclick` strings in templates                     | Declarative `x-data` / `@click` reads better than ad-hoc inline JS; pairs naturally with HTMX (Alpine for local state, HTMX for server round-trips) |
...
Library choices locked: skip Tailwind, Node toolchain, `django-crispy-forms`, `django-cotton`, Google Fonts CDN, `django-htmx-modal` (Pico styles `<dialog>`). Add `django-browser-reload` (dev dep group only). Vendor Pico CSS v2, HTMX, Alpine.js 3.x, and Inter (not Python deps; self-hosted under `static/`). Template partials use Django 6.0's built-in tags - no `django-template-partials` package needed.
