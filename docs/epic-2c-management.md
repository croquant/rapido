...
| Navigation IA             | Top-bar always; left rail only on `/settings/*`. Routes under `/o/<slug>/`: `/`, `/settings/locations/`, `/settings/members/`, `/settings/organization/`. Members and invitations share one page (pending invitations, members, revoked/expired archive stacked); invitation action endpoints stay at `/settings/invitations/<pk>/{resend,revoke}/` (no list page). Profile at `/me/` (not tenant-scoped); accept at `/invite/<token>/` (anonymous-accessible) | Flat IA at SMB scale; invitations and members are the same admin task — splitting them caused context flipping; profile is identity, not tenant data |
...
1. ADMIN clicks "Invite" on `/o/<slug>/settings/members/` -> Alpine `dialog.showModal()` opens an inline `<dialog>` pre-rendered with the `InvitationForm`.
...
4. Response: 204 + `HX-Redirect` to `/settings/members/` so pending and archive sections re-render together; flash via `HX-Trigger: invitations:created`.
